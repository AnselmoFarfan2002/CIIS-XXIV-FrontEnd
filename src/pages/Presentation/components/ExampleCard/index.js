...
function ImageZoom({ src, alt, className }) {
  ...
  useEffect(() => {
    ...
    zoom.on("open", () => {
      imageRef.current.style.padding = "20px 20px 20px 20px"; // Aplica el padding al hacer zoom
      imageRef.current.style.transition = "0.5s";
      document.querySelector("#navbar").classList.add("zoom-out");
    });

    zoom.on("close", () => {
      imageRef.current.style.padding = "0px"; // Aplica el padding al hacer zoom
      document.querySelector("#navbar").classList.remove("zoom-out");
    });

    return () => {
      zoom.detach(); // Desvincula el zoom cuando se desmonta el componente
    };
  }, []);
